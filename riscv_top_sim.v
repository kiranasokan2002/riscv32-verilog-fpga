// ============================================================
// RISC-V Top Level — SIMULATION VERSION
// No VIO — rst as input port
// Use this for behavioral simulation only
// ============================================================
module riscv_top_sim (
    input  clk,
    input  rst,
    output [7:0]  led_result,
    output [3:0]  led_opcode,
    output [3:0]  led_pc,
    output [3:0]  led_regval,
    output [31:0] dbg_pc,
    ...
    imm_gen u_immgen (.instruction(instruction),.imm_out(imm));
    control_unit u_ctrl (
    ...
    alu u_alu (.A(alu_a),.B(alu_b),.ALUCtrl(ALUCtrl),.Result(alu_result),.Zero(Zero));
    data_memory u_dmem (
        .clk(clk),.rst(rst),
        .MemRead(MemRead),.MemWrite(MemWrite),
        .addr(alu_result),.WriteData(read_data2),.ReadData(mem_read_data));

    assign write_back_data = Jump?pc_plus4:MemToReg?mem_read_data:alu_result;

    assign dbg_pc         = pc;
    assign dbg_instr      = instruction;
    assign dbg_alu_result = alu_result;
    assign dbg_rd_data    = write_back_data;
    assign led_result     = alu_result[7:0];
    assign led_pc         = pc[5:2];
    assign led_regval     = u_regfile.regs[3][3:0];
